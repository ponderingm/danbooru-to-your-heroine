...
ヒロイン定義（`heroines` 設定の `identity`, `dna`）を設計・提案・実装する際は、以下の原則を厳守すること。

1. **基本構成は「キャラクタータグ ＋ 維持して欲しい最小限の特徴」のみ**:
   - ヒロインDNAは、全身の装飾や服・ポーズ・表情を細かく羅列する場所ではない。
   - **キャラクタータグ**（モデルの学習知識を呼び出す核）に加え、元絵の影響で崩れやすい・絶対に維持したい**最小限の特徴（肌色、髪型、瞳色、胸部サイズ、固有の身体特徴）**のみで構成する。
   - *設計の具体例（例: 褐色・小胸ヒロインを定義する場合）*:
     - キャラクタータグ: `character_name (series_name)` 等のBooru標準キャラクタータグ（※ `@` はアーティストタグ記法のためキャラ名には付与しない）
     - 最小限の維持DNA: `small breasts`, `dark skin`, `tanlines` 等の崩れやすい核となる身体特徴
     - それ以外の「衣装」「ポーズ」「表情」「背景」などはDNAに固定せず、Danbooruの元絵（または選択した衣装アドオン）に委ねる。
2. **過剰指定の禁止（トークン予算と自由度の保護）**:
   - SDXL / Illustrious の CLIP テキストエンコーダーは、**先頭75トークン（第1チャンク）**が最も強力なアテンションを持つ。
   - ヒロインDNAを盛りすぎると、元絵の魅力的な衣装・構図・背景を表現するためのトークン枠を圧迫し、どんな元絵を持ってきても「いつもの服・いつものポーズ」に引っ張られて換装の面白さが失われる。
   - 「盛る」のではなく「削ぎ落として核だけを残す（引き算の美学）」を徹底すること。
...
- **Booruサイトの検索特性と「共起継承の輪（xxxxx ring）」**:
  - **Danbooru（起爆剤と共起の積集合）**: 無料枠/匿名では「最大2タグまで」のAPI制限があり、`word1 word2 sort: rating:` 等の3タグ以上の複合検索はAPI側で弾かれる。クライアント側でのページ繰り手元フィルタは可能だが時間がかかり実用的ではない。しかし**タグ付け品質はBooru界最高峰**であり、わずか1単語（例: 微小装飾や特定モチーフ）から極めて濃厚な装飾・情景タグが数珠つなぎに付与されているため、**「少数精鋭のクリティカルワードから神絵師の共起構造をまるごと吸い上げる」**運用において真価を発揮する。
  - **Gelbooru（AND検索による複数条件の多角的な絞り込み）**: 無料でも3タグ以上の複数タグ・`sort:score`・`rating:` を含む複合AND検索を柔軟に受け付ける。タグ付けの厳密さではDanbooruに譲るものの、複数の条件タグを重ね合わせてピンポイントに絞り込み、手早くバッチ生成を回す運用に最適。
